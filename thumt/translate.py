...
from pr_utils import getPRBatch, feature_word, feature_phrase, \
					feature_length, feature_attention_coverage, feature_wordcount, feature_senmemory, feature_treememory
...
if __name__ == "__main__":
	...
	if config['PR']:
		...
		for fl in config['features_PR']:
			...
			#print(fl)
			#print("feature nums: "+str(len(fls)))
			model = eval(config['model'])(config, fls = fls)
			model.build()
		'''

	print("model build complished!")
	try:
