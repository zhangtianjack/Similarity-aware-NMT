...
class RNNsearch(model):
	...
	def decode_sample(self, state_init, c, length, n_samples):
		...
		state = tensor.repeat(state_init, n_samples, axis = 0)  # copy state n times
	...
	def build(self, verbose = False):
		...
		self.emb_src = self.creater.createLookupTable(self.name + 'emb_src',
			config['num_vocab_src'], config['dim_emb_src'], offset = True) #(input,output)-->[30000,620]
		self.emb_trg = self.creater.createLookupTable(self.name + 'emb_trg',
			config['num_vocab_trg'], config['dim_emb_trg'], offset = True)  #(input,output)-->[30000,620]
		...
		if self.fls:
			...
			self.feature_weight_dim = self.feature_weight.dimshuffle('x', 0)  # equal to a.T  (m,n)-->(n,m)
