...
def shuffle(src, trg, src_shuf, trg_shuf, data_corpus = 'json'):
	...
	# train fms
	logging.info('Shuffle trees\' fuzzy match score')
	with open("/home/tfzhang/corpus/hansard/train_treefms/zman/hansard_zman_zh.fms",'r') as xfms:
		with open("/home/tfzhang/corpus/hansard/train_treefms/zman/hansard_zman_en.fms", 'r') as yfms:
			before_xfms = xfms.readlines()
			before_yfms = yfms.readlines()
			after_xfms=[]
			after_yfms=[]
			for i in range(len(idx)):
				if (i + 1) % 100000 == 0:
					logging.debug(str(i + 1))
				index = idx[i]
				after_xfms.append(before_xfms[index])
				after_yfms.append(before_yfms[index])
			with open("/home/tfzhang/experiment_x/hansard_zmantree/corpus/xfms.shuf", 'wb') as f:
				json.dump(after_xfms, f)
			with open("/home/tfzhang/experiment_x/hansard_zmantree/corpus/yfms.shuf", 'wb') as f:
				json.dump(after_yfms, f)
