...
class feature_treememory(featureList):
		...
		if lang=='en':
			for i, line in enumerate(self.enfcorpus):
				for word in line.split():
					mydict.setdefault(word, []).append(i)
		return mydict
	def pretrans(self,ntree):  # preprocess of tree
		posright = 0
		posspace = 0
	...
	def compare(self,cursen,curtree,lang):
		if cursen=="" or curtree.decode('utf-8')=='))' or re.match("Warning|The",curtree.decode('utf-8')):
			return 0
		trees = self.DiceSearch(cursen,lang)
		fms =0
	...
	def solrsearch(self,curtree,lang):
		...
		if lang=="zh":
			url = "http://localhost:8983/solr/hansard_zhtree/select?q=target:\"" + curtree + "\""
		elif lang=="en":
			url = "http://localhost:8983/solr/hansard_entree/select?q=target:\"" + curtree + "\""
	...
	def getFeatures(self, x, y, xfms,yfms,add_info = None):
		...
		if add_info !=2:    # 0,1 represent training , 2 represent inference
			#ogging.info(xfms[0][:-2].decode("utf-8"))
			#logging.info(len(xfms))


			#logging.info(str(xfms))
			#logging.info(str(xfms[0][:-1]))
			...
			if add_info== 1:
				...
				#logging.info("sample: " + seny)
				ytree = self.berkeleyparser(seny,'en')
				ysiml = self.compare(tempy,ytree,'en')
				#logging.info("ysiml: "+str(ysiml))

			else:
				ysiml = float(yfms[0][:-1])
				#logging.info("y: " + seny)
			#logging.info("xsiml: " + str(xsiml)+" , ysiml: "+ str(ysiml))
			return numpy.asarray([1.0 * xsiml, 1.0 * ysiml], dtype='float32')

		elif add_info==2:
			#logging.info("tune process")
			senx = ""
			#logging.info(x)
			#logging.info(y)
			for index in x[:-1]:
			...
			xsiml = self.compare(tempx,xtree,'zh')
			#logging.info("dev xsiml: " + str(xsiml))
			tempy=seny
			seny = re.sub('\(', '-LRB-', seny)
			seny = re.sub('\)', '-RRB-', seny)
			ytree = self.berkeleyparser(seny,'en')
			ysiml = self.compare(tempy,ytree, 'en')
			#logging.info("dev ysiml: " + str(ysiml))
			return numpy.asarray([1.0 * xsiml, 1.0 * ysiml], dtype='float32')
